  ...
  describe("With multiple donors & grantee", () => {
    ...
    describe(`Handling correct dilution for fund -> approve refund -> withdraw refund 
      ...
      before(async () => {
        ...
        await tokenFromSecondDonor.approve(grantFromDonorWithToken.address, 1e6);
      });
      ...
      it("should update balance of donor and grant on withdraw refund to multiple donors", async () => {
        ...
        // first donor
        const balanceBeforeRefundForDonor = await _token.balanceOf(await _donorWallet.getAddress());
        await _grantFromDonor.withdrawRefund(await _donorWallet.getAddress());
        const balanceAfterRefundForDonor = await _token.balanceOf(await _donorWallet.getAddress());
        expect(balanceBeforeRefundForDonor.add(REFUND_AMOUNT / 2)).to.eq(balanceAfterRefundForDonor);

        // second donor
        const balanceBeforeRefundForSecondDonor = await _token.balanceOf(await _secondDonorWallet.getAddress());
        await _grantFromDonor.withdrawRefund(await _secondDonorWallet.getAddress());
        const balanceAfterRefundForSecondDonor = await _token.balanceOf(await _secondDonorWallet.getAddress());
        expect(balanceBeforeRefundForSecondDonor.add(REFUND_AMOUNT / 2)).to.eq(balanceAfterRefundForSecondDonor);
      ...
      it("should update balances of multiple grantees and Grant on payout & withdrawl", async () => {
        ...
        // first grantee
        const balanceBeforePayoutForGrantee = await _token.balanceOf(await _granteeWallet.getAddress());
        await _grantFromManager.approvePayout(REFUND_AMOUNT, await _granteeWallet.getAddress());
        await _grantFromManager.withdrawPayout(await _granteeWallet.getAddress());
        const balanceAfterPayoutForGrantee = await _token.balanceOf(await _granteeWallet.getAddress());
        expect(balanceBeforePayoutForGrantee.add(REFUND_AMOUNT)).to.eq(balanceAfterPayoutForGrantee);

        // second grantee
        const balanceBeforePayoutForSecondGrantee = await _token.balanceOf(await _secondGranteeWallet.getAddress());
        await _grantFromManager.approvePayout(REFUND_AMOUNT, await _secondGranteeWallet.getAddress());
        await _grantFromManager.withdrawPayout(await _secondGranteeWallet.getAddress());
        const balanceAfterPayoutForSecondGrantee = await _token.balanceOf(await _secondGranteeWallet.getAddress());
        expect(balanceBeforePayoutForSecondGrantee.add(REFUND_AMOUNT)).to.eq(balanceAfterPayoutForSecondGrantee);

        // Checking Grant's token balance
        const balanceAfterPayoutForGrant = await _token.balanceOf(_grantFromManager.address);
        expect(balanceBeforePayoutForGrant.sub(REFUND_AMOUNT).sub(REFUND_AMOUNT)).to.eq(balanceAfterPayoutForGrant);
      });
      ...
      it("should update balance of donor and grant on withdraw refund a second time", async () => {
        ...
        expect(balanceBeforeRefundForGrant.sub(REFUND_AMOUNT / 2)).to.eq(balanceAfterRefundForGrant);
      });
    });
    ...
    describe("Refunding - Approve & Withdraw", () => {
      describe("With Token - total funding by both donors at once", () => {
        ...
        before(async () => {
          ...
          const { refunded: _lastRefundOfDonor } = await _grantFromManager.donors(await _donorWallet.getAddress());
          const { refunded: _lastRefundOfSecondDonor } = await _grantFromManager.donors(await _secondDonorWallet.getAddress());
          lastRefundOfDonor = _lastRefundOfDonor;
          lastRefundOfSecondDonor = _lastRefundOfSecondDonor;

          tokenBalanceOfDonor = await _token.balanceOf(await _donorWallet.getAddress());
          tokenBalanceOfSecondDonor = await _token.balanceOf(await _secondDonorWallet.getAddress());
        });
        it("should be refunded with initial amount", async () => {
          await _grantFromManager.approveRefund(REFUND_AMOUNT, AddressZero);

          await _grantFromManager.withdrawRefund(await _donorWallet.getAddress());
          await _grantFromManager.withdrawRefund(await _secondDonorWallet.getAddress());

          // Checking current refund balance
          const { refunded: _lastRefundOfDonor } = await _grantFromManager.donors(await _donorWallet.getAddress());
          const { refunded: _lastRefundOfSecondDonor } = await _grantFromManager.donors(await _secondDonorWallet.getAddress());

          expect(lastRefundOfDonor.add(REFUND_AMOUNT / 2)).to.be.eq(_lastRefundOfDonor);
          expect(lastRefundOfSecondDonor.add(REFUND_AMOUNT / 2)).to.be.eq(_lastRefundOfSecondDonor);

          // Checking current token balance
          const _tokenBalanceOfDonor = await _token.balanceOf(await _donorWallet.getAddress());
          const _tokenBalanceOfSecondDonor = await _token.balanceOf(await _secondDonorWallet.getAddress());

          expect(tokenBalanceOfDonor.add(REFUND_AMOUNT / 2)).to.be.eq(_tokenBalanceOfDonor);
          expect(tokenBalanceOfSecondDonor.add(REFUND_AMOUNT / 2)).to.be.eq(_tokenBalanceOfSecondDonor);
      ...
      describe("With Ether - total funding by both donors at once", () => {
        ...
        before(async () => {
          ...
          await _donorWallet.sendTransaction({ to: grantFromDonorWithEther.address, value: FUND_AMOUNT, gasPrice: 1 });
          await _secondDonorWallet.sendTransaction({ to: grantFromDonorWithEther.address, value: FUND_AMOUNT, gasPrice: 1 });

          const { refunded: _lastRefundOfDonor } = await _grantFromManagerWithEther.donors(await _donorWallet.getAddress());
          ...
          etherBalanceOfDonor = await _provider.getBalance(await _donorWallet.getAddress());
          etherBalanceOfSecondDonor = await _provider.getBalance(await _secondDonorWallet.getAddress());
        });
        ...
        it("should be refunded with initial amount", async () => {
          ...
          const receiptForDonor = await (
            await _grantFromDonorWithEther.withdrawRefund(await _donorWallet.getAddress(), { gasPrice: 1 })
          ).wait();
          const receiptForSecondDonor = await (
            await _grantFromSecondDonorWithEther.withdrawRefund(await _secondDonorWallet.getAddress(), { gasPrice: 1 })
          ).wait();
          ...
          const _etherBalanceOfDonor = await _provider.getBalance(await _donorWallet.getAddress());
          const _etherBalanceOfSecondDonor = await _provider.getBalance(await _secondDonorWallet.getAddress());

          expect(etherBalanceOfDonor.add(REFUND_AMOUNT / 2).sub(receiptForDonor.gasUsed)).to.be.eq(_etherBalanceOfDonor);
          expect(etherBalanceOfSecondDonor.add(REFUND_AMOUNT / 2).sub(receiptForSecondDonor.gasUsed)).to.be.eq(

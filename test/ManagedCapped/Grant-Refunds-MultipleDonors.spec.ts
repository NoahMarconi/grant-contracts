  ...
  describe("With multiple donors & grantee", () => {
    ...
    describe(`Handling correct dilution for fund -> approve refund -> withdraw refund 
      ...
      it("should update balances of multiple grantees and Grant on payout & withdrawl", async () => {
        ...
        await _grantFromManager.approvePayout(250, _granteeWallet.address);
        ...
        expect(balanceBeforePayoutForGrantee.add(250)).to.eq(balanceAfterPayoutForGrantee);

        // second grantee
        const balanceBeforePayoutForSecondGrantee = await _token.balanceOf(_secondGranteeWallet.address);
        await _grantFromManager.approvePayout(250, _secondGranteeWallet.address);
        ...
        expect(balanceBeforePayoutForSecondGrantee.add(250)).to.eq(balanceAfterPayoutForSecondGrantee);

        // Checking Grant's token balance
        const balanceAfterPayoutForGrant = await _token.balanceOf(_grantFromManager.address);
        expect(balanceBeforePayoutForGrant.sub(250).sub(250)).to.eq(balanceAfterPayoutForGrant);
      });

      it("should update balance of donor and grant on withdraw refund a second time", async () => {

        const _REFUND_AMOUNT = 250;


        const balanceBeforeRefundForGrant = await _token.balanceOf(_grantFromManager.address);

        await _grantFromManager.approveRefund(_REFUND_AMOUNT, AddressZero);

        // refunding by first donor and balance calculations
        const balanceBeforeRefundForDonor = await _token.balanceOf(_donorWallet.address);
        await _grantFromDonor.withdrawRefund(_donorWallet.address);
        const balanceAfterRefundForDonor = await _token.balanceOf(_donorWallet.address);
        expect(balanceBeforeRefundForDonor.add(_REFUND_AMOUNT/2)).to.eq(balanceAfterRefundForDonor);

        const balanceAfterRefundForGrant = await _token.balanceOf(_grantFromManager.address);

        expect(balanceBeforeRefundForGrant.sub(_REFUND_AMOUNT/2)).to.eq(balanceAfterRefundForGrant);
      });
    });

  ...
  describe("With Token", () => {
    describe("Approve Payout", () => {
      ...
      it("should revert if someone other than manager tries to approve payout", async () => {
        await expect(_grantFromDonorWithToken.approvePayout(_fundAmount, await _granteeWallet.getAddress())).to.be.revertedWith(
      ...
      it("should revert if target funding != total funding", async () => {
        await expect(_grantFromManagerWithToken.approvePayout(_fundAmount, await _granteeWallet.getAddress())).to.be.revertedWith(
      ...
      it("should revert if value > target funding", async () => {
        const { targetFunding } = await _grantFromManagerWithToken.grantees(await _granteeWallet.getAddress());
        await expect(_grantFromManagerWithToken.approvePayout(targetFunding + 1, await _granteeWallet.getAddress())).to.be.revertedWith(
      ...
      it("should revert if address does not belongs to grantee", async () => {
        await expect(_grantFromManagerWithToken.approvePayout(_payoutAmount, await _unknownWallet.getAddress())).to.be.revertedWith(
          "approvePayout::Invalid Argument. value cannot exceed remaining allocation."
        );
      });

      it("should emit LogPaymentApproval on approve payment", async () => {
        await expect(_grantFromManagerWithToken.approvePayout(_payoutAmount, await _granteeWallet.getAddress()))
      ...
      it("should emit LogPayment on payment withdrawl", async () => {
        await expect(_grantFromManagerWithToken.withdrawPayout(await _granteeWallet.getAddress()))
      ...
      it("should update total payed of grantee and Grant", async () => {
        ...
        const { totalPaid } = await _grantFromManagerWithToken.grantees(await _granteeWallet.getAddress());
    ...
    describe("Grantee balance", () => {
      ...
      it("should not be updated yet", async () => {
        const tokenBalance = await _tokenFromDonor.balanceOf(await _granteeWallet.getAddress());
      ...
      it("should updated with token after payout withdrawl", async () => {
        await _grantFromManagerWithToken.approvePayout(_payoutAmount, await _granteeWallet.getAddress());
        await _grantFromManagerWithToken.withdrawPayout(await _granteeWallet.getAddress());
  ...
  describe("With Ether", () => {
    describe("Grantee balance", () => {
      ...
      before(async () => {
        ...
        _initialEtherBalance = await _provider.getBalance(await _granteeWallet.getAddress());
      });

      it("should not be updated yet", async () => {
        const etherBalance = await _provider.getBalance(await _granteeWallet.getAddress());
        expect(_initialEtherBalance).eq(etherBalance);

        const { totalPaid } = await _grantFromManagerWithEther.grantees(await _granteeWallet.getAddress());
        expect(0).to.eq(totalPaid);
      });

      it("should updated approved amount approve payout", async () => {
        await _grantFromManagerWithEther.approvePayout(_payoutAmount, await _granteeWallet.getAddress());

        const { payoutApproved } = await _grantFromManagerWithEther.grantees(await _granteeWallet.getAddress());
        expect(_payoutAmount).eq(payoutApproved);

      });

      it("should updated ether after withdrawl", async () => {
        await _grantFromManagerWithEther.withdrawPayout(await _granteeWallet.getAddress());

        const { totalPaid } = await _grantFromManagerWithEther.grantees(await _granteeWallet.getAddress());
        expect(_payoutAmount).eq(totalPaid);

        const etherBalanceAfterPayout = await _provider.getBalance(await _granteeWallet.getAddress());
    ...
    describe("With Token", () => {
      describe("Grantees' balance", () => {
        ...
        before(async () => {
          ...
          await tokenFromDonor.approve(grantFromDonorWithToken.address, TARGET_FUNDING_1);
          await _grantFromDonor.fund(TARGET_FUNDING_1);

          const { totalPaid: initialtotalPaidForGrantee } = await _grantFromManager
            .grantees(await _granteeWallet.getAddress());
          ...
          const { totalPaid: initialtotalPaidForSecondGrantee } = await _grantFromManager
            .grantees(await _secondGranteeWallet.getAddress());

          _lasttotalPaidForSecondGrantee = initialtotalPaidForSecondGrantee;
        });

        it("should revert if approved amount is 0", async () => {
          await expect(_grantFromManager.approvePayout(0, await _granteeWallet.getAddress()))
            .to.be.reverted;
        });
        ...
        it("should update payoutApproved & totalPaid on first approval / withdrawl", async () => {
          ...
          await _grantFromManager.approvePayout(approveAmount, await _granteeWallet.getAddress());
          ...
          } = await _grantFromManager.grantees(await _granteeWallet.getAddress());
          ...
          await _grantFromManager.approvePayout(approveAmount, await _secondGranteeWallet.getAddress());
          ...
          } = await _grantFromManager.grantees(await _secondGranteeWallet.getAddress());
    ...
    describe("With Ether", () => {
      describe("Grantees' Balance", () => {
        ...
        before(async () => {
          ...
          etherBalanceOfGrantee = await _provider.getBalance(await _granteeWallet.getAddress());

          const {
            totalPaid: _totalPaidOfGrantee,
            payoutApproved: payoutApprovedGrantee
          } = await _grantFromManagerWithEther.grantees(await _granteeWallet.getAddress());

          totalPaidOfGrantee = _totalPaidOfGrantee;

          etherBalanceOfSecondGrantee = await _provider.getBalance(await _secondGranteeWallet.getAddress());
          
          const {
            totalPaid: _totalPaidOfSecondGrantee,
            payoutApproved: payoutApprovedSecondGrantee
          } = await _grantFromManagerWithEther.grantees(await _secondGranteeWallet.getAddress());
        ...
        it("should be updated with initial payout", async () => {
          // first grantee
          await _grantFromManagerWithEther.approvePayout(900, await _granteeWallet.getAddress());
          await _grantFromManagerWithEther.withdrawPayout(await _granteeWallet.getAddress());

          const _etherBalanceOfGrantee = await _provider.getBalance(await _granteeWallet.getAddress());
          ...
          // second grantee
          await _grantFromManagerWithEther.approvePayout(300, await _secondGranteeWallet.getAddress());
          await _grantFromManagerWithEther.withdrawPayout(await _secondGranteeWallet.getAddress());
          ...
          const _etherBalanceOfSecondGrantee = await _provider.getBalance(await _secondGranteeWallet.getAddress());
        ...
        it("should be updated with final payout", async () => {
          // first grantee
          await _grantFromManagerWithEther.approvePayout(100, await _granteeWallet.getAddress());
          await _grantFromManagerWithEther.withdrawPayout(await _granteeWallet.getAddress());
          ...
          // second grantee
          await _grantFromManagerWithEther.approvePayout(200, await _secondGranteeWallet.getAddress());
          await _grantFromManagerWithEther.withdrawPayout(await _secondGranteeWallet.getAddress());

...
contract Grant is AbstractGrant, ISignal, ReentrancyGuard {
    ...
    uint256 public constant GRAINS = 10 ** 18;
        ...
        public
    {
        ...
        if (!isManager(msg.sender)) {
            ...
            //      1. Funding goal is not met before fundingExpiration.
            //      2. Funds are not completely dispersed before contractExpiration.
            require(
                // solium-disable-next-line security/no-block-members
        ...
        returns(bool)
    {
        ...
        uint256 percentContributed = donors[donor].funded
            .mul(GRAINS).div( // @audit-ok one thing to consider might be that tokens with exceptionally high decimal case digits might break this. Even though there are no such cases in the wild (sensible ones, at least) the standard does not forbid it
                totalFunding
            );
